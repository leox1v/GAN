...
def main(_):
    ...
    for it in range(FLAGS.max_iter * int(1E3)):
        ...
        _, D_loss_curr, _, G_loss_curr, _, _, _,summary = sess.run(
            [model.d_solver, model.D_loss, model.g_solver, model.G_loss, model.d_gradients, model.g_gradients, model.Q_loss,
             model.merged], feed_dict={model.X: X, model.lat: lat})
    ...
    plt.savefig(FLAGS.output_dir + 'final_info.png', bbox_inches='tight')
    plt.close(fig)
